...
class Settings extends MX_Controller
{
    ...
    public function general_page_configuration()
    {
        ...
        $data['config'] = $this->SettingsModel->get_cilent_config(); //?
    ...
    public function update_general_page()
    {
        ...
        $data['maintenance_mode'] = isset($data['maintenance_mode']) && $data['maintenance_mode'] ? '1' : '0';
        $data['limit_access_mode'] = isset($data['limit_access_mode']) && $data['limit_access_mode'] ? '1' : '0';
        //var_dump($data);exit;
        ...
        $this->form_validation->set_rules('maintenance_end_time', 'Lama Waktu Maintenance', 'required');
        $this->form_validation->set_rules('max_limit', 'Max Limit Akses', 'required');
        $this->form_validation->set_rules('limit_access_end_time', 'Lama Limit Akses', 'required');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if ($edit == true) {
                $this->SettingsModel->update_maintenance(1, $data);
                $this->SettingsModel->update_limit_access(1, $data);
