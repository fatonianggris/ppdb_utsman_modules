...
class CostModel extends CI_Model
{
    ...
    public function update_cost_guide($value = '', $id = '')
    {
        $this->db->trans_begin();

        $data = array(
            'nama_panduan' => $value['nama_panduan'],
            'keterangan_panduan' => $value['keterangan_panduan'],
            'file_panduan' => $value['file_panduan'],
            'updated_at' => date("Y-m-d H:i:s"),
        );

        $this->db->where('id_panduan_biaya', $id);
        $this->db->update($this->table_cost_guide, $data);
