...
class SettingsModel extends CI_Model
{
    ...
    private $table_client_page   = 'konfigurasi';
    private $table_contact       = 'kontak';
    private $table_third_party   = 'third_party_ppdb';
    private $table_configuration = 'konfigurasi';
    ...
    public function get_cilent_config()
    {

        $this->db->select('*');
        $this->db->where('id_konfigurasi', 1);
        $sql = $this->db->get($this->table_client_page);
        return $sql->result();
    }

    public function update_maintenance($id = '', $value = '')
    {
        $this->db->trans_begin();

        $data = [
            'maintenance_mode'     => $value['maintenance_mode'],
            'maintenance_end_time' => $value['maintenance_end_time'],
            'updated_at'           => date("Y-m-d H:i:s"),
        ];

        $this->db->where('id_konfigurasi', $id);
        $this->db->update($this->table_configuration, $data);
    ...
    public function update_limit_access($id = '', $value = '')
    {
        $this->db->trans_begin();

        $data = [
            'limit_access_mode'     => $value['limit_access_mode'],
            'max_limit'             => $value['max_limit'],
            'limit_access_end_time' => $value['limit_access_end_time'],
            'updated_at'            => date("Y-m-d H:i:s"),
        ];
    ...
    public function update_contact($id = '', $value = '')
    {
        ...
        $data = [
            ...
            'grup_wa_dc_kb_tk' => @$value['grup_wa_dc_kb_tk'],
            'grup_wa_sd_reg'   => @$value['grup_wa_sd_reg'],
            'grup_wa_sd_icp'   => @$value['grup_wa_sd_icp'],
            'grup_wa_smp'      => @$value['grup_wa_smp'],
            'grup_wa_sma'      => @$value['grup_wa_sma'],
            'email'            => $value['email'],
