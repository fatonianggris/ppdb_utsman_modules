...
class BankAccount extends MX_Controller {
    ...
    public function post_bank_account() {
        ...
        if ($check == TRUE) {
            ...
            redirect('ppdb/payment/bankaccount/add_bank_account');
        } else {
            //print_r($param);exit;
            if ($this->form_validation->run() == FALSE) {

                $this->session->set_flashdata('flash_message', warn_msg(validation_errors()));
                redirect('ppdb/payment/bankaccount/add_bank_account'); //folder, controller, method
            } else {
    ...
    public function update_bank_account($id = '') {
        ...
        if ($check == TRUE && $data['nama_alatbayar'] != $get_name[0]->nama_alatbayar) {
            ...
            redirect('ppdb/payment/bankaccount/edit_bank_account/' . paramEncrypt($id));
        } else {
    ...
    public function activate_bank_account() {
        ...
        if ($activate == true) {
            ...
            redirect('ppdb/payment/bankaccount/list_bank_account');
        } else {
            $this->session->set_flashdata('flash_message', err_msg('Maaf, Terjadi kesalahan...'));
            redirect('ppdb/payment/bankaccount/list_bank_account');
        }
